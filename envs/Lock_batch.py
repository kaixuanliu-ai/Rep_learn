...
class LockBatch(gym.Env):
    ...
    def init(self,horizon=100, action_dim=10, p_switch=0.5, p_anti_r=0.5, anti_r=0.1,noise=0.1, num_envs=10, temperature=1, 
        ...
        self.observation_space = Box(low=0.0, high=1.0, shape=(self.observation_dim,),dtype=np.float64)
        ...
        print([int(z) for z in self.opt_a])
        print("[LOCK] B sequence: ", end="")
        print([int(z) for z in self.opt_b])
    ...
    def step(self,action):
        ...
        next_state = np.zeros(self.num_envs, dtype=np.int32)
    ...
    def reset(self, bad=False):
        ...
        if bad:
            self.state = np.zeros(self.num_envs, dtype=np.int32)
            for i in range(self.num_envs):
